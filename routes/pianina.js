...
router.get('/', function (req, res, next) {
    res.render('pianina', { title: 'Piano4U - Pianina', pianina: pianina, year: '2023', year: '2023', blad:null  });
});
router.post('/', function (req, res) {
    ...
    dobazy().then((db) => {
        const newRecord = { ID: null, imie: zamowienie.first, nazwisko: zamowienie.last, email: zamowienie.email, okres: zamowienie.okres, model: zamowienie.model, data: new Date() };
        db.query('INSERT INTO zamowienia SET? ', newRecord, function (error, results) {
            if (error) throw error;
            console.log(results);
        });
        res.render('pianina', {userValue: zamowienie, title: 'Piano4U - Pianina', pianina: pianina,year: '2023', blad:null });

    }).catch((err) => {
        res.render('pianina', {userValue: zamowienie, title: 'Piano4U - Pianina', pianina: pianina,year: '2023', blad:err });
        console.error(err);}
    );
